	...
	pub fn do_phragmen() -> Vec<(T::AccountId, BalanceOf<T>, BalanceOf<T>)> {
		...
		let _ = sp_npos_elections::seq_phragmen::<T::AccountId, Perbill>(
			...
			exposed_candidates.into_iter().for_each(|c| {
				...
				{
					let (imbalance, _leftover) = T::Currency::slash_reserved(&c, T::CandidacyBond::get());
					use frame_support::traits::Imbalance;
					slashed_outgoing.push((c.clone(), imbalance.peek(), _leftover));
